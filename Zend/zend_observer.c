...
zend_llist zend_observers_fiber_switch_callbacks;
...
ZEND_API void zend_observer_startup(void) {
	...
	zend_llist_init(&zend_observers_fiber_switch_callbacks, sizeof(zend_observer_fiber_switch_handler), NULL, 1);
}
...
ZEND_API void zend_observer_shutdown(void) {
	...
	zend_llist_destroy(&zend_observers_fiber_switch_callbacks);
}
...
{
	zend_llist_add_element(&zend_observers_fiber_switch_callbacks, &handler);
}
...
void zend_observer_fiber_switch_notify(zend_fiber *from, zend_fiber *to)
{
	...
	for (element = zend_observers_fiber_switch_callbacks.head; element; element = element->next) {
		handler = *(zend_observer_fiber_switch_handler *) element->data;
		handler(from, to);
	}
}
